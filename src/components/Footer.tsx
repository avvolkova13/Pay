...
const publicAssetBase = process.env.GITHUB_ACTIONS === "true" ? "/Pay" : "";

function PaymentMethodLogo({ label }: { label: string }) {
  if (label === "UnionPay") {
    return (
      <img
        className="payment-icon payment-icon-unionpay"
        src={`${publicAssetBase}/payment-methods/unionpay.svg`}
        alt=""
        aria-hidden="true"
      />
    );
  ...
  if (label in simpleIconPaths) {
    return (
      <svg className={`payment-icon payment-icon-${label.toLowerCase().replace(/\s+/g, "-")}`} viewBox="0 0 24 24" focusable="false" aria-hidden="true">
        <path d={simpleIconPaths[label as keyof typeof simpleIconPaths]} />
      </svg>
    );
  }
